    ...
    function switchTab(tab) {
        const target = tab === 'tpn' ? 'tpn' : 'en';
        const sectionEn = document.getElementById('section-en');
        const sectionTpn = document.getElementById('section-tpn');
        const sectionRef = document.getElementById('section-ref');
        const tabEn = document.getElementById('tab-en');
        const tabTpn = document.getElementById('tab-tpn');
        const tabRef = document.getElementById('tab-ref');

        if (sectionEn) sectionEn.classList.toggle('hidden', target !== 'en');
        if (sectionTpn) sectionTpn.classList.toggle('hidden', target !== 'tpn');
        if (sectionRef) sectionRef.classList.add('hidden');
        if (tabEn) tabEn.classList.toggle('active', target === 'en');
        if (tabTpn) tabTpn.classList.toggle('active', target === 'tpn');
        if (tabRef) tabRef.classList.remove('active');

        const content = document.getElementById('monitoring-content-actual');
        if (!content) return;
        const container = document.querySelector(
            target === 'en' ? '.monitoring-container-shared-en' : '.monitoring-container-shared-tpn'
        );
        if (container && content.parentElement !== container) {
...
async function saveCurrentSupportDay(){
  ...
  const {data,error}=await supabase.from('nutrition_support_days')
    ...
    ;
  if(error){console.error('nutrition_support_days UPDATE failed:',error);alert('تعذر حفظ بيانات هذا اليوم: '+(error.message||'تحقق من صلاحية UPDATE في RLS.'));return false;}
  const i=supportDays.findIndex(x=>x.id===d.id);
  if(i>=0)supportDays[i]={...supportDays[i],state,updated_at:new Date().toISOString()};
...
$('tab-en')?.addEventListener('click',()=>switchTab('en'));
$('tab-tpn')?.addEventListener('click',()=>switchTab('tpn'));
