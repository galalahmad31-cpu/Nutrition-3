...
async function savePatient(e){
  ...
  try{
    let data,error;
    if(id){
      ({data,error}=await supabase.from('patients').update(payload).eq('id',id).eq('user_id',currentUser.id).select('id,name,gender,birth_date,age,height,diagnosis,complaints,clinical_notes').single());
    }else{
      ({data,error}=await supabase.from('patients').insert({...payload,user_id:currentUser.id}).select('id,name,gender,birth_date,age,height,diagnosis,complaints,clinical_notes').single());
    }
...
async function initNutritionSupport() {
  try {
    if (!access || !supabase) throw new Error('تعذر تهيئة نظام الدخول.');
    ...
    console.error(e);
    $('patientEmpty').textContent = 'تعذر تحميل قائمة المرضى';
    $('patientEmpty').style.display = 'block';
  }
}

initNutritionSupport();
