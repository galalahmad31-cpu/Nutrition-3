(function () {
  ...
  async function init() {
    ...
    $('patientGrid').addEventListener('click', (event) => {
      ...
      if (edit) { event.stopPropagation(); const patient = patients.find((item) => item.id === edit.dataset.edit); if (patient) openPatientModal(patient); return; }
      const del = event.target.closest('[data-delete]');
      if (del) { event.stopPropagation(); deletePatient(del.dataset.delete); }
    });
    ...
    $('patientModalOverlay').addEventListener('click', (event) => { if (event.target === $('patientModalOverlay')) closePatientModal(); });
    document.addEventListener('keydown', (event) => { if (event.key === 'Escape') closePatientModal(); });
  }
