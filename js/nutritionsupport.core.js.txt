(function () {
  ...
  async function refreshNutritionSupportAccess() {
    try {
      const accessApi = window.DietPlannerCoreAccess;
  ...
  async function init() {
    ...
    supabase = window.DietPlannerSupabase?.client || null;

    if (!access || !supabase) {
      console.error('DietPlannerAccess is not available.');
      const empty = $('patientEmpty');
      if (empty) {
        empty.textContent = 'تعذر تشغيل صفحة الدعم الغذائي. تأكد من تحميل ملفات Core.';
