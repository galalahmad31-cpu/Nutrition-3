(function () {
  ...
  function niceStep(range, targetTicks = 5) {
    const raw = Math.max(range, 0.1) / targetTicks;
  ...
  function renderWeightChart(logs) {
    ...
    const minY = Math.max(0, Math.floor((minValue - step * 0.6) / step) * step);
    const maxY = Math.ceil((maxValue + step * 0.6) / step) * step;
    ...
    const linePath = points.map((point, index) =>
      `${index === 0 ? 'M' : 'L'} ${x(index)} ${y(point.weight)}`
    ).join(' ');
    ...
    box.innerHTML = `
      <div class="chart-shell">
        <svg viewBox="0 0 ${width} ${height}" class="weight-chart" role="img" aria-label="منحنى تطور الوزن">
          ...
          <line id="chartFocusLine" x1="0" y1="${pad.top}" x2="0" y2="${pad.top + plotH}" class="chart-focus"/>
    ...
    function showPoint(index) {
      ...
      const svgPointX = x(index);
      const svgPointY = y(point.weight);
      const shellWidth = shell.clientWidth || 1;
      const renderedX = (svgPointX / width) * shellWidth;
      const renderedY = (svgPointY / height) * shell.clientHeight;

      focusLine.setAttribute('x1', svgPointX);
      focusLine.setAttribute('x2', svgPointX);
      focusDot.setAttribute('cx', svgPointX);
      focusDot.setAttribute('cy', svgPointY);
    ...
    box.querySelector('.weight-chart')?.addEventListener('mousemove', (event) => {
      const rect = event.currentTarget.getBoundingClientRect();
      const svgX = ((event.clientX - rect.left) / rect.width) * width;
      const nearest = Math.max(0, Math.min(points.length - 1, Math.round(((svgX - pad.left) / plotW) * (points.length - 1))));
      showPoint(nearest);
    });

    box.querySelector('.weight-chart')?.addEventListener('mouseleave', hidePoint);
  }
