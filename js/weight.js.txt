(function () {
  ...
  function renderWeightChart(logs) {
    ...
    // Keep about five full intervals below the lowest measured weight.
    // This gives the curve visual breathing room and makes the change over time
    // easier to read without exaggerating the slope.
    const lowerIntervals = 5;
    const minY = Math.max(
      0,
      Math.floor(minValue / step) * step - (lowerIntervals * step)
    );
  ...
  function bindEvents() {
    ...
    const axisStep = $('weightAxisStep');
    axisStep?.addEventListener('change', () => renderWeightChart(state.weightLogs));
  }
