(function () {
  ...
  function getActivePlan() {
    ...
    return {
      subscription,
      plan: state.plans.find((item) => item.id === subscription.plan_id) || null
    };
  }

  function renderSubscriptionStatus() {
    const container = $('subscriptionStatus');
    if (!container) return;
    ...
    const planName = active.plan?.name || 'الخطة الحالية';
    const expiry = active.subscription.expiry_date || '';

    container.innerHTML = `
      <div class="subscription-status-card">
        <div class="status-icon"><i class="fa-solid fa-circle-check"></i></div>
        <div class="status-copy">
          <strong>أنت مشترك حاليًا في ${escapeHtml(planName)}</strong>
          <div class="status-meta">${expiry ? `الاشتراك ساري حتى ${escapeHtml(expiry)}` : 'الاشتراك مفعّل حاليًا'}</div>
        </div>
        <div class="status-action">الحالة: مفعّل</div>
      </div>`;
  }
