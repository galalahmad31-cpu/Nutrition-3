...
const state = {
  ...
  search: '',
  isAdmin: false,
  editor: { mode: null, productId: null, formulas: [], deletedFormulaIds: [] },
  pendingDelete: null,
...
function closeEditor() {
  ...
  state.editor = { mode: null, productId: null, formulas: [], deletedFormulaIds: [] };
}
...
$('formulaList').addEventListener('click', e => {
  ...
  const removed = formulas[index];
  formulas.splice(index, 1);
  renderFormulaEditor();

  if (removed?.id) {
    state.editor.deletedFormulaIds.push(removed.id);
  }
});
