/* =========================================================
 * VISIT PAGE
 * Single feature script for visit.html.
 * Supabase client/auth are provided by auth-access.js.
 * ========================================================= */
...
function openPrintSettingsModal(){
 ...
 if(pb)pb.onclick=()=>window.dietPlan.executePrint();
...
async function ensureExchangePlan(){
 ...
 S.plan=null;
 const plans=await findPlans(),base=plans.find(p=>p.plan_name==='الخطة الغذائية')||plans.find(p=>n(p.target_calories)>0);if(!base)return null;S.t={cal:n(base.target_calories),pro:n(base.target_protein),carb:n(base.target_carb),fat:n(base.target_fat)};const id=crypto.randomUUID();const payload={id,patient_id:base.patient_id,visit_id:base.visit_id||ctx().id||null,plan_name:'الخطة الغذائية باستخدام البدائل',start_date:new Date().toISOString().slice(0,10),target_calories:S.t.cal,target_protein:S.t.pro,target_carb:S.t.carb,target_fat:S.t.fat,target_fluid:base.target_fluid??null,goal:base.goal??null,notes:base.notes??null};const r=await dbx.from('nutrition_plans').insert(payload).select('id').single();if(r.error){status('تعذر إنشاء خطة البدائل: '+r.error.message,true);return null}S.plan=r.data.id;return S.plan}
async function loadDays(){if(!S.plan){S.days=[];S.savedDays=[];renderDays();return}const {data:dr,error:de}=await dbx.from('plan_days').select('*').eq('plan_id',S.plan).order('day_number',{ascending:true});if(de){status('تعذر تحميل أيام خطة البدائل: '+de.message,true);return}const ids=(dr||[]).map(x=>x.id);let ms=[];if(ids.length){const r=await dbx.from('plan_meals').select('*').in('day_id',ids).order('meal_order',{ascending:true});if(r.error){status('تعذر تحميل وجبات الخطة: '+r.error.message,true);return}ms=r.data||[]}const mids=ms.map(x=>x.id);let its=[];if(mids.length){const r=await dbx.from('plan_items').select('id,meal_id,food_id,quantity_g,household_measure,frequency,item_name').in('meal_id',mids);if(r.error){status('تعذر تحميل أصناف الخطة: '+r.error.message,true);return}its=r.data||[]}S.days=(dr||[]).map(d=>({id:d.id,title:d.day_name||'اليوم',meals:ms.filter(m=>m.day_id===d.id).map(m=>({id:m.id,name:m.meal_name||'وجبة',items:its.filter(i=>i.meal_id===m.id).map(i=>({id:i.id,name:i.item_name||'',measure:i.household_measure||'',repeat:i.frequency||''}))}))}));S.savedDays=clone(S.days);S.dayEditing={};renderDays()}
...
function openPrintSettings(){
 ...
 const pb=document.getElementById('executePrintBtn');
 if(!modal||!pb)return;
 pb.onclick=()=>window.exchangePlan.saveAndPrintFromSettings();
 modal.classList.remove('hidden');
}
function printAll(){
 ...
 * The two diet modules intentionally share one confirmation modal.
 * Only this single listener owns the confirmation button.
 * ========================================================= */
