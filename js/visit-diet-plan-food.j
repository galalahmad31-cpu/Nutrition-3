...
function filterFoodList(){
 ...
 const q=String(input.value||'').trim().toLocaleLowerCase('ar-EG');
 const selectedId=String(document.getElementById('selectedFoodId')?.value||'');
 const list=q?foodDatabase.filter(f=>String(f.name||'').toLocaleLowerCase('ar-EG').includes(q)):foodDatabase;
 if(!list.length){drop.innerHTML='<div class="p-3 text-xs text-slate-400 text-center font-bold">لا توجد نتائج مطابقة</div>';return;}
 drop.innerHTML=list.map(f=>{
  const id=String(f.id),selected=id===selectedId;
  return `<button type="button" class="food-result w-full text-right p-2.5 hover:bg-emerald-50 cursor-pointer flex justify-between items-center text-xs ${selected?'bg-emerald-100/70 border-r-4 border-emerald-600':''}" data-food-id="${escapeHtml(id)}"><span><span class="font-extrabold text-slate-800 block">${escapeHtml(f.name)}</span><span class="text-[10px] text-slate-500 font-semibold">${num(f.calories)} kcal | بروتين ${num(f.protein)}g | كارب ${num(f.carbs)}g | دهون ${num(f.fat)}g | Na ${num(f.sodium)}mg | K ${num(f.potassium)}mg | P ${num(f.phosphorus)}mg</span>${f.household?`<span class="text-[10px] text-emerald-700 font-bold block mt-0.5">100 جم ≈ ${escapeHtml(f.household)}</span>`:''}</span>${selected?'<i class="fa-solid fa-circle-check text-emerald-600"></i>':'<i class="fa-solid fa-plus text-slate-300"></i>'}</button>`;
 }).join('');
}
...
function initFoodSearchInteraction(){
 const drop=document.getElementById('foodDropdownList'); if(!drop||drop.dataset.bound==='1')return;
 drop.addEventListener('click',event=>{const result=event.target.closest('.food-result'); if(!result)return; event.preventDefault(); selectFoodForMeal(result.dataset.foodId);});
...
async function init(){
 ...
 if(!foodsLoaded){dietInitialized=false;return;}
 await loadPlan();
}
