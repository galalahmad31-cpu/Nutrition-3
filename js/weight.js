(function () {
  ...
  function renderWeightChart(logs) {
    ...
    const values = logs.map(item => Number(item.weight)).filter(Number.isFinite);
    ...
    const step = niceStep(dataRange, 5);
    const minY = Math.max(0, Math.floor((minValue - step * 0.5) / step) * step);
    const maxY = Math.ceil((maxValue + step * 0.5) / step) * step;
    const yRange = Math.max(maxY - minY, step);

    const x = (index) => logs.length === 1
      ? pad.left + plotW / 2
      : pad.left + (index / (logs.length - 1)) * plotW;
    const y = (value) => pad.top + ((maxY - value) / yRange) * plotH;

    const ticks = [];
    for (let value = minY; value <= maxY + step / 100; value += step) {
      ticks.push(Number(value.toFixed(2)));
    }

    const grid = ticks.map((value) => `
      <line x1="${pad.left}" y1="${y(value)}" x2="${width - pad.right}" y2="${y(value)}" class="chart-grid"/>
      <text x="${pad.left - 10}" y="${y(value) + 4}" text-anchor="end" class="chart-axis">${value}</text>
    `).join('');

    const points = logs.map((item, index) => `${x(index)},${y(Number(item.weight))}`).join(' ');
    const areaPoints = `${pad.left},${pad.top + plotH} ${points} ${x(logs.length - 1)},${pad.top + plotH}`;

    const labels = logs.map((item, index) => {
      const show = logs.length <= 6 || index === 0 || index === logs.length - 1 || index % Math.ceil(logs.length / 5) === 0;
      return show
        ? `<text x="${x(index)}" y="${height - 16}" text-anchor="middle" class="chart-date">${escapeHtml(formatDate(item.date))}</text>`
        : '';
    }).join('');

    const dots = logs.map((item, index) => `
      <circle cx="${x(index)}" cy="${y(Number(item.weight))}" r="5" class="chart-dot">
        <title>${escapeHtml(formatDate(item.date))}: ${Number(item.weight).toFixed(1)} كجم</title>
      </circle>
    `).join('');

    const linePath = logs.map((item, index) => `${index ? 'L' : 'M'} ${x(index)} ${y(Number(item.weight))}`).join(' ');
    ...
    box.innerHTML = `
      <svg viewBox="0 0 ${width} ${height}" class="weight-chart" role="img" aria-label="منحنى تطور الوزن">
        ...
        ${grid}
        <polygon points="${areaPoints}" fill="url(#weightAreaGradient)"/>
        <path d="${linePath}" class="chart-line"/>
        ${dots}
        ${labels}
        <text x="16" y="${pad.top + plotH / 2}" transform="rotate(-90 16 ${pad.top + plotH / 2})" text-anchor="middle" class="chart-title">الوزن (كجم)</text>
      </svg>`;
  }
