...
function getLastVisitDate(finance = {}) {
    // An actual recorded visit always has priority.
    // For an old record without last_visit_date, the subscription
    // start date is treated as the initial actual visit.
    return finance.last_visit_date || finance.subscription_start_date || null;
}

function getNextFollowUpDate(finance = {}, followUpDays = null, startDate = null) {
    ...
    const baseDate = getLastVisitDate({
        ...finance,
        subscription_start_date: startDate || finance.subscription_start_date
    });
...
function updateFollowUpPreview() {
    ...
    const days = $('followUpDays').value;

    const lastVisit = getLastVisitDate({
        ...finance,
        subscription_start_date: $('startDate').value || finance.subscription_start_date
    });

    $('lastVisit').textContent = lastVisit ? fmt(lastVisit) : '—';
...
function openFinanceModal(row) {
    ...
    const lastVisit = getLastVisitDate(finance);
    $('lastVisit').textContent = lastVisit ? fmt(lastVisit) : '—';

    // When opening an existing record, show its saved appointment.
    // When no appointment exists yet, calculate a preview from the
    // actual last visit (or initial subscription date).
    $('nextFollowUp').textContent = finance.next_follow_up_date
        ? fmt(finance.next_follow_up_date)
        : fmt(getNextFollowUpDate(
            finance,
            $('followUpDays').value,
            $('startDate').value
        ));
...
async function saveFinance() {
    ...
    try {
        const existingFinance = selected.finance || {};

        // Existing actual visit must never be replaced by editing
        // subscription data. For a first-time record, the subscription
        // start date becomes the initial actual visit.
        const lastVisitDate =
            existingFinance.last_visit_date ||
            startDate ||
            null;

        const nextFollowUpDate =
            lastVisitDate && followUpDays
                ? addDays(lastVisitDate, followUpDays)
                : null;
...
function bindEvents() {
    ...
    $('startDate').addEventListener('input', () => {
        ...
        // Only the initial visit follows a changed start date.
        // Existing actual visit remains untouched.
        if (!selected?.finance?.last_visit_date) {
            $('lastVisit').textContent = $('startDate').value
                ? fmt($('startDate').value)
                : '—';
        }
...
async function init() {
    try {
        if (!supabase || !access?.getCurrentUser) {
